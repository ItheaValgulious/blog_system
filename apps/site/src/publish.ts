...
async function readPublishConfig(): Promise<PublishConfig> {
...
async function commitAndPush(
  ...
  await runGitCommand(["push", "origin", `HEAD:${branch}`], worktreeDir, secretToMask);
...
const publishConfig = await readPublishConfig();
...
await ensureRemoteState(tempDir, remoteUrl, publishConfig.deployBranch ?? defaultBranch, publishConfig.authToken ?? "");
